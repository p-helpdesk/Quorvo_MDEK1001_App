...
abstract class NetworkNodeBleConnectionImpl implements NetworkNodeBleConnection {
    ...
    private final Consumer<String> onDisconnectingListener;
    ...
    NetworkNodeBleConnectionImpl(@NotNull GattInteractionFsm fsm,
                                 ...
                                 @NotNull Consumer<String> onDisconnectingListener,
    ...
    public void disconnect() {
        ...
        onDisconnectingListener.accept(bleAddress);
...
//        }, token);
    ...
    public void getOtherSideEntity(@NotNull Consumer<NetworkNode> onSuccess,
        ...
        if (properties != null && properties.length > 0) {
            ...
            if (nodeType == null) {
                // make sure that nodetype and node id (bugfix for #254) is known
                builder.addProperty(NetworkNodeProperty.NODE_TYPE);
                builder.addProperty(NetworkNodeProperty.ID);
            }
    ...
    private <T extends DataChangedCallback> void genericObserveData(final T dataChangedCallback,
        ...
        if (nodeType == null) {
            decodeAndSaveNodeType(dataChangedCallback::onFail, true);
            if (nodeType == null) {
                // nodetype is still null
                // we need to fetch the nodeType/operationMode first
                token = ReadCharacteristicOperation.enqueue(gattOperationQueue,
                        new ReadCharacteristicRequest(BleConstants.SERVICE_UUID_NETWORK_NODE, BleConstants.CHARACTERISTIC_OPERATION_MODE),
                        (synchronousBleGatt1 -> {
                            // onSuccess
                            if (!decodeAndSaveNodeType(dataChangedCallback::onFail, false)) {
                                dataChangedCallback.onFail(new Fail(ErrorCode.GATT_REPRESENTATION,
                                        "there is still missing nodeType even after operationMode fetch"));
                            }
                        }),
                        (synchronousBleGatt1, fail) -> callFailConditionalDisconnect(dataChangedCallback::onFail, fail),
                        getGattDecoder().getContext(),
                        token    // possible dependency
                );
            }
        }
        // cccd always goes first, then it is asynchronously confirmed by locationDataNotificationSet
    ...
    private List<WriteCharacteristicRequest> convertNetworkNodeToWriteRequests(NetworkNode networkNode, boolean nodeTypeChanged) {
        ...
        boolean needsOperationMode = nodeTypeChanged || diffNode.isUwbModeChanged() || diffNode.isOperatingFirmwareChanged()
                || diffNode.isFirmwareUpdateEnableChanged() || diffNode.isLedIndicationEnableChanged() || diffNode.isBleEnableChanged();
