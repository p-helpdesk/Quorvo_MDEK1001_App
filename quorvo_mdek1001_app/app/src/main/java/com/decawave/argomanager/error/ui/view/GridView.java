...
package com.decawave.argomanager.error.ui.view;

import static com.decawave.argomanager.ArgoApp.daApp;
import static com.decawave.argomanager.ArgoApp.uiHandler;
import static com.decawave.argomanager.error.ui.DisplayMetrics.LCD_DENSITY_SCALING_FACTOR;
import static com.decawave.argomanager.error.ui.DisplayMetrics.LCD_DIP_SCALING_FACTOR;
...
import com.decawave.argomanager.error.ui.DisplayMetrics;
...
public class GridView extends View {
    ...
    private void construct(Context ctx) {
        ...
        tagColorCache = CacheBuilder.newBuilder().build(new CacheLoader<Long, Integer>() {
            @Override
            public Integer load(@NonNull Long key) throws Exception {
                return Util.computeColorForNodeId(key);
            }
        });
    ...
    private void drawNode(Canvas canvas, NetworkNode node, List<NodeAndPosition> visibleNodes, int tagAlpha) {
        Position p = getNodePosition(node);
        if (p == null) {
            // skip this one, there is no position value
            return;
        }
        mapPositionToFout(p);
    ...
    private void drawTag(Canvas canvas, float x, float y, float closeToScreenEdgeFactor, TagNode tag, int alpha) {
        Position position = getNodePosition(tag);
        if (DEBUG_DRAW_NODE) {
    ...
    private void drawTagRangingAnchors(Canvas canvas, TagNode tagNode, int lineAlpha, int textAlpha) {
        ...
        Position p = getNodePosition(tagNode);
        if (p == null) {
            // skip this one
            return;
        }
    ...
    private int computeDistance(TagNode tag, Position anchorPosition) {
        Position position = getNodePosition(tag);
        // position must be non-null (we are drawing distance)
        // deltas
        int xDelta = Math.abs(position.x - anchorPosition.x);
        int yDelta = Math.abs(position.y - anchorPosition.y);
        int zDelta = Math.abs(position.z - anchorPosition.z);
        // compute distance
    ...
    private int getColorForTag(NetworkNode tagNode) {
        try {
            return tagColorCache.get(tagNode.getId());
...
//        float virtualHeightCm = cmHeight * INITIAL_ZOOM_RATIO;
    ...
    public void onNodeChanged(NetworkNode newNode) {
        ...
        if (getNodePosition(myNode) != null) {
            // the node had a z-axis, remove the node first
            nodesByZAxis.remove(myNode);
        }
        //
        removeCachedDebugStrings(myNode.getBleAddress());
        // copy properties to 'myNode'
        if (myNode.getType() != newNode.getType()) {
        ...
        if (getNodePosition(newNode) != null) {
            // add 'myNode' again - properly sorted according to the possibly new z-axis
    ...
    public void onNodeAdded(NetworkNode node) {
        ...
        if (Constants.DEBUG) {
            ...
            // make sure that we did not have this node previously in avg map
            Preconditions.checkState(!avgNodesById.containsKey(node.getId()), "node " + node.getId() + " already contained in AVG map!");
        }
        if (node.isTag()) {
            initTagAvg(node);
        }
        if (getNodePosition(node) != null) nodesByZAxis.add(node);
        // and now redraw
        invalidate();
    }
