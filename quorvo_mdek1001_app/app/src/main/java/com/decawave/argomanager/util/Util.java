...
public class Util {
    ...
    public static boolean isRealInitiator(Integer macStats) {
        return macStats != null && (macStats & (1 << 3)) != 0;
    }
    ...
    public static int computeColorForNodeId(long nodeId) {
        // take only the lowest 2 bytes
        nodeId = 0x00FFFF & nodeId;
        // one of these compounds always resolves to 0 (we have only short nodeId)
        short b0 = (short) (nodeId & 0x00ff);
        short b1 = (short) ((nodeId & 0x00ff00) >> 8);
        short b2 = (short) ((nodeId & 0x00ff0000) >> 16);
        short h = shortHash(b0, b1, b2);

        int baseColor = baseColors[h % baseColors.length];

        int shortBaseR = (baseColor & 0x00FF0000) >> 16;
        int shortBaseG = (baseColor & 0x0000FF00) >> 8;
        int shortBaseB = baseColor & 0x000000FF;

        short red = (short) (Math.round((shortBaseR + b0) * h / 255.0) % 255);
        short green = (short) (Math.round((shortBaseG + b1) * h / 255.0) % 255);
        short blue = (short) (Math.round((shortBaseB + b2) * h / 255.0) % 255);
        // the resulting color is here
        return 0xFF000000 | (red << 16 | green << 8 | blue);
    }


    public static int computeColorForAddressOld(String bleAddress) {
        // parse the address
    ...
    public static void applyStyle(Activity mainActivity, TextView textView, int extraStyle) {
