...
class PolymorphicDiscoveryList {
    ...
    interface PersistedNetworkResolver {
        ...
        boolean isNetworkPersisted(short networkId);
    ...
    class DeclaredNetworkListItem extends DiscoveryListBean {
        ...
        DeclaredNetworkListItem(short networkId) {
            ...
            if (Constants.DEBUG) {
                Preconditions.checkState(persistedNetworkResolver.isNetworkPersisted(networkId));
            }
            this.networkId = networkId;
        }
    ...
    void onDeclaredNetworkRemoved(short networkId) {
        // try to find the network and remove it, if applicable
        ListIterator<DiscoveryListBean> it = list.listIterator();
        while (it.hasNext()) {
            DiscoveryListBean item = it.next();
            if (item.type == ItemType.DECLARED_NETWORK && networkId == ((DeclaredNetworkListItem) item).networkId) {
                removeItemBehindIterator(it, item);
            }
        }
    }
