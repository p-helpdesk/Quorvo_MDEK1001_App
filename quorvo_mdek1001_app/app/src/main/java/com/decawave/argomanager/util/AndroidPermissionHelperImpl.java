...
public class AndroidPermissionHelperImpl implements AndroidPermissionHelper {
    ...
    public boolean mkSureServicesEnabledAndPermissionsGranted(MainActivity mainActivity, @NotNull Action0 grantSuccessListener, @NotNull Action0 grantFailListener) {
        if (Constants.DEBUG) {
            ...
            Preconditions.checkNotNull(grantSuccessListener, "grant success listener must not be null");
            Preconditions.checkNotNull(grantFailListener, "grant fail listener must not be null");
        }
        if (!allSetUp()) {
    ...
    private void nextStep(MainActivity mainActivity) {
        ...
        if (!locationServiceEnabled()) {
            ...
            genericStartActivityForResult(mainActivity, REQUEST_CODE_ENABLE_BT);
    ...
    public void startActivityToEnableLocationService(MainActivity mainActivity) {
        genericStartActivityForResult(mainActivity, REQUEST_CODE_ENABLE_LOCATION);
    }
    ...
    private static void genericStartActivityForResult(MainActivity mainActivity, int requestCode) {
        Intent i;
        switch (requestCode) {
