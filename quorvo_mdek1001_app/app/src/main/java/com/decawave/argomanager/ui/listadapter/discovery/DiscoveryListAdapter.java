...
public class DiscoveryListAdapter extends FlexibleAdapter<IFlexible>{
    ...
    private PolymorphicDiscoveryList createPolymorphicList() {
        PolymorphicDiscoveryList list = new PolymorphicDiscoveryList(new PolymorphicDiscoveryList.PersistedNetworkResolver() {
            ...
            @Override
            public boolean isNetworkPersisted(short networkId) {
                return networkNodeManager.getNetworks().get(networkId) != null;
            }

            @Override
            public String getNetworkName(short networkId) {
                NetworkModel network = networkNodeManager.getNetworks().get(networkId);
                if (network == null) {
                    throw new IllegalStateException("invalid network id: " + Util.formatAsHexa(networkId));
                }
    ...
    public void onNetworkRemoved(short networkId) {
        if (Constants.DEBUG) {
            log.d("onNetworkRemoved: " + "networkId = [" + networkId + "]");
        }
        polymorphicList.onDeclaredNetworkRemoved(networkId);
    }
